...
function renderReader(title, sub, content, id, isNote) {
    ...
    const deleteFileBtn = isNote
        ? `<button onclick="deleteNote('${id}')" class="btn-delete"><i class="fas fa-trash"></i> Supprimer Note</button>`
        : '';
    ...
    const authorSection = isNote
        ? `<div style="display:flex;align-items:center;gap:8px;margin-top:6px;">
               ...
               <input type="text" id="note-author-${id}" value="${sub}" placeholder="Ajouter un auteur..."
                   style="border:1px solid var(--border);border-radius:8px;padding:4px 10px;font-size:13px;color:var(--text-main);background:#fff;font-family:Inter;width:200px;">
           </div>`
    ...
    document.getElementById('main-reader').innerHTML = `
        ...
        <div id="delegation-result-${id}" style="display:none;background:#fff;border:1px solid var(--delegate);border-radius:16px;padding:20px;margin-top:20px;">
...
function createQuickTag(messageId) {
    ...
    if (!label) return;
    input.disabled = true; input.value = "Création...";
    fetch('/tags/create-ajax', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'label=' + encodeURIComponent(label) })
        .then(res => res.json())
        .then(data => { if (data.error) { alert(data.error); input.disabled = false; input.value = label; } else { upd(messageId, data.tag, false); } })
        .catch(() => { alert("Erreur de connexion"); input.disabled = false; });
}
...
function startDelegation(messageId) {
    ...
    fetch('/delegate-auto', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'messageId=' + encodeURIComponent(messageId) })
        .then(r => r.json())
        .then(data => {
            ...
            btn.onclick = () => confirmDelegationAction(messageId, assignee, document.getElementById('del-draft-' + messageId).value);
        })
        .catch(() => { document.getElementById('del-draft-' + messageId).value = "Erreur IA."; });
}

function confirmDelegationAction(messageId, assignee, draftBody) {
    showLoader("Création du brouillon...");
    fetch('/delegate-confirm', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'messageId=' + encodeURIComponent(messageId) + '&assignee=' + encodeURIComponent(assignee) + '&draftBody=' + encodeURIComponent(draftBody) })
        .then(r => r.json()).then(() => { window.location.href = "/kanban"; });
}
...
function setFilter(tag, chipEl) {
    ...
    document.querySelectorAll('.feed-list .mail-card').forEach(card => {
        ...
        let show = tag === 'ALL' ? true : tag === 'PENDING' ? (cardTag === 'PENDING' || cardTag === '' || cardTag === 'null') : tag === 'DO' ? cardTag === 'DO' : cardTag === tag;
...
function toggleSidebarLayout() {
    ...
    localStorage.setItem('eisenflow_layout', app.classList.contains('topbar-mode') ? 'topbar' : 'sidebar');
}
